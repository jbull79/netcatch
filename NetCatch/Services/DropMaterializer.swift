...
///
/// Real files are used in place. File representations and raw image/data drops — e.g.
/// an image dragged from Safari/Preview or a pasted screenshot (which macOS names
...
enum DropMaterializer {
    ...
    /// Any item with a file representation (incl. most file promises) — the system
    /// materializes it into a temp it controls and hands us a URL we copy into our
    /// container, so we never depend on a sandbox-forbidden write location.
